...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $InputCantidad = $input['cantidad'] ?? null;

    // Verifica que todos los parámetros estén presentes
    if ($InputIdUsuario && $InputIdProducto && $InputCantidad) {
        ...
        $stmt->bind_param("sii", $InputIdUsuario, $InputIdProducto, $InputCantidad);

        // Ejecuta la declaración
        if ($stmt->execute()) {
            // Si la inserción fue exitosa, devuelve una respuesta exitosa
            echo json_encode(['message' => 'Producto añadido al carrito correctamente']);
        } else {
            // En caso de error al ejecutar el procedimiento
            echo json_encode(['error' => 'Error al agregar al carrito: ' . $stmt->error]);
        }
